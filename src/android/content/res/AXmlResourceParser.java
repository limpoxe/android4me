...
import android4me.res.ReadUtil;
 ...
 * to next() and retains until setInput(), close(), or failed call to next().
 * (2) Closed state, which parser obtains after setInput(), close(), or failed
...
public class AXmlResourceParser implements XmlResourceParser {
	...
	public void setInput(InputStream stream,String inputEncoding) throws XmlPullParserException {
		...
		if (inputEncoding!=null) {
			throw new XmlPullParserException("Input encoding is not supported (must be null).");
		}
	...
	public int next() throws XmlPullParserException,IOException {
		if (m_reader==null) {
			throw new XmlPullParserException("Parser doesn't have any input.",this,null);
		}
		try {
			doNext();
			return m_event;			
	...
	public String getName() {
		...
		return m_strings.getRaw(m_name);
	}
	...
	public String getNamespace() {
		return m_strings.getRaw(m_namespaceUri);
	}
	
	public String getPrefix() {
		int prefix=m_namespaces.findPrefix(m_namespaceUri);
		return m_strings.getRaw(prefix);
	}
	...
	public String getAttributeNamespace(int index) {
		...
		return m_strings.getRaw(namespace);
	}
	...
	private final void doNext() throws IOException {
		...
		if (m_strings==null) {
			ReadUtil.readCheckType(m_reader,CHUNK_AXML_FILE);
